<template>
  <v-data-table
    :headers="headers"
    :items="grades"
    sort-by="calories"
    class="elevation-1"
  >

            <template v-slot:[`item.student_id`]="{ item }">
              <span>{{ item.student.lastname}}</span>
            </template>

             <template v-slot:[`item.subject_id`]="{ item }">
              <span>{{ item.subject.name}}</span>
            </template>
            
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Grades</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
        ></v-divider>
        <v-spacer></v-spacer>

        <!-- AddDialog -->
        <v-dialog
          v-model="dialog"
          max-width="500px"
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              color="primary"
              dark
              class="mb-2"
              v-bind="attrs"
              v-on="on"
            >
              Add
            </v-btn>
          </template>
          <v-card>
            <v-card-title>
              <span class="text-h5">{{ formTitle }}</span>
            </v-card-title>

            <v-card-text>
              <v-container>
                <v-row>
                  <v-col
                    cols="12"
                    sm="6"
                    md="6"
                  >
                    <!-- <v-text-field
                      v-model="formItem.lastname"
                      label="Enter Time"
                    ></v-text-field> -->
                    <v-select
                    :items="students"
                    v-model="formItem.student_id"
                     item-value="id" 
                     item-text= lastname
                     label="Student"
                    >
                    </v-select>

                  </v-col>

                             <v-col
                    cols="12"
                    sm="6"
                    md="6"
                  >
                    <!-- <v-text-field
                      v-model="formItem.contact"
                      label="Enter Mobile Number"
                    ></v-text-field> -->
                      <v-select
                    :items="subjetcs"
                    v-model="formItem.subject_id"
                     item-value="id" 
                     item-text= name
                     label="Subject"
                    >
                     </v-select>
                    
                  </v-col>

                  <v-col
                    cols="12"
                    sm="6"
                    md="6"
                  >
                    <v-text-field
                      v-model="formItem.semester1"
                      label="Enter Student Grade"
                    ></v-text-field>
                  </v-col>

                   <v-col
                    cols="12"
                    sm="6"
                    md="6"
                  >
                    <v-text-field
                      v-model="formItem.semester2"
                      label="Enter Student Grade"
                    ></v-text-field>
                  </v-col>
               
             
                </v-row>
              </v-container>
            </v-card-text>

            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn
                color="blue darken-1"
                text
                @click="close"
              >
                Cancel
              </v-btn>
              <v-btn
                color="blue darken-1"
                text
                @click="saveGrade"
              >
                Save
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
          
          <!-- DeleteConfirmDialog -->
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5">Are you sure you want to delete this item?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="closeDelete">Cancel</v-btn>
              <v-btn color="blue darken-1" text @click="deleteGrade(item)">OK</v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
          </v-card>
        </v-dialog>
          
          <!-- ViewDialog -->
           <v-dialog
      v-model="dialogView"
      width="500"
    >
      <v-card>
        <v-card-title class="text-h5 grey lighten-2">
          {{formItem.semester1}}
        </v-card-title>

        <v-card-text>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.
        </v-card-text>

        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
            color="primary"
            text
            @click="dialogView = false"
          >
            Close
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    
      </v-toolbar>
    </template>
    <template v-slot:[`item.actions`] ="{ item }">
     
       <v-icon
        small
        class="mr-2"
        @click="viewGrade(item)"
      >
        mdi-eye
      </v-icon>
      <v-icon
        small
        class="mr-2"
        @click="editGrade(item)"
      >
        mdi-pencil
      </v-icon>
      <v-icon
        small
        class="mr-2"
        @click="deleteConfirm(item)"
      >
        mdi-delete
      </v-icon>
    </template>
    <template v-slot:no-data>
    </template>
  </v-data-table>
</template>


<script>
  export default {
    data: () => ({
      dialog: false,
      dialogDelete: false,
      dialogView: false,
      headers: [
        {
          text: 'ID',
          align: 'start',
          sortable: false,
          value: 'id',
        },
        {text: 'Student', value:'student_id'},
        {text: 'Subject', value: 'subject_id' },
        {text: '1st Semester', value: 'semester1'},
        {text: '2nd Semester', value: 'semester2'},
        {text: 'Final Remark', value: 'final_remark'},
        { text: 'Actions', value: 'actions', sortable: false },
      ],
      students:[],
      subjetcs:[],
      grades:[],
      final:{},
      plus:{},
      result:{},
      

      // item:{},
      editedIndex: -1,
      formItem: {
        semester1:'',
        semester2:'',
        final_remark:''
      },
    }),

    computed: {
      formTitle () {
        return this.form === -1 ? 'Add' : 'Add Classes'
      },
    },

    watch: {
        dialogView (val) {
        val || this.viewDialogClose()
      },
      dialog (val) {
        val || this.close()
      },
      dialogDelete (val) {
        val || this.closeDelete()
      },
      
      dialog (val){
      if(val){
        axios.get('api/student/index').then(response => {
            this.students =response.data
        })
         axios.get('api/subject/index').then(response => {
            this.subjetcs =response.data
        })
        }
      }
    },

    created () {
      this.initialize()
    },

    methods: {
      initialize () {
         axios.get('api/grade/index')
         .then(response => {
          this.grades = response.data
         }).catch(error => {
          console.log(error)
         })
      },

      close () {
        this.defaultItem()
        this.dialog = false
        this.$nextTick(() => {
          this.editedItem = Object.assign({}, this.defaultItem)
          this.editedIndex = -1
        })
      },

      closeDelete () {
        this.defaultItem()
        this.dialogDelete = false
        this.$nextTick(() => {
          this.editedItem = Object.assign({}, this.defaultItem)
          this.editedIndex = -1
        })
      },

      save () {
        if (this.editedIndex > -1) {
          Object.assign(this.desserts[this.editedIndex], this.editedItem)
        } else {
          this.desserts.push(this.editedItem)
        }
        this.close()
      },

      saveGrade() {
        if(this.formItem.id)
        {
             axios.put('api/grade/update/'+this.formItem.id, this.formItem)
             .then(response => {
                 this.initialize()
                 this.defaultItem()
                 this.close()
             })
        }
        else
        {
        axios.post('api/grade/create/', this.formItem)
        .then(response => {
                 this.initialize()
                 this.defaultItem()
                 this.close()
             })
        .catch(function (error) {
          console.log(error);
        });
        this.initialize();
        this.close()
        }
      },

      viewGrade(item)
      {
        this.dialogView = true
         axios.get('api/grade/'+item.id)
        .then(response=> {
          this.formItem = response.data
        })
      },

      viewDialogClose()
      {
        this.defaultItem()
        this.dialogView = false
      },

      editGrade(item)
      {
        this.Title = 'Edit Category'
        this.formItem = JSON.parse(JSON.stringify(item))
        this.average()

        //   axios.get('api/classes/'+item.id)
        // .then(response=> {
        //   this.formItem = response.data
        // })

        this.dialog = true
      },

      deleteGrade(item)
      {
        axios.delete('api/grade/delete/'+item.id)
        this.initialize();
        this.closeDelete();
      },

      deleteConfirm(item)
      {
        //  axios.delete('api/student/delete/'+item.id)
        // this.initialize();
        this.dialogDelete = true
        this.item = item
      },

      defaultItem()
      {
        this.formItem.id = ''
        this.formItem.student_id  = ''
        this.formItem.subject_id = ''
        this.formItem.semester1 = ''
        this.formItem.semester2 = ''
        this.formItem.final_remark = ''
      },

      average()
      {
         this.plus = parseInt(this.formItem.semester1)+parseInt(this.formItem.semester2)
         this.result = parseInt(this.plus)/5+50
         this.final = this.result
         console.log(this.final)
      }

    },
  }
</script>