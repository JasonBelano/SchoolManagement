        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="deleteSubject(item)">OK</v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
<script>
  export default {
    ...
    computed: {
      formTitle () {
        return this.form === -1 ? 'Add' : 'Add Subject'
      },
    },
    ...
    methods: {
      ...
      saveSubject() {
        if(this.formItem.id)
        {
             axios.put('api/subject/update/'+this.formItem.id, this.formItem)
             .then(response => {
                 this.initialize()
                 this.defaultItem()
                 this.close()
        ...
        {
        axios.post('api/subject/create', this.formItem)
      ...
      deleteSubject(item)
      {
        axios.delete('api/subject/delete/'+item.id)
        this.initialize();
        this.closeDelete();
